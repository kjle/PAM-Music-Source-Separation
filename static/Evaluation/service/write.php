...
// I add this
// echo json_encode($session->trials[0]->responses[0], JSON_PRETTY_PRINT);
$stimulusRating = $session->trials[0]->responses[0]->stimulus;
if (!is_array($stimulusRating)) {
    // error_log("Warning: stimulusRating is not an array, converting...");
    $stimulusRating = [$stimulusRating]; // 转换为数组
}
$ratingCount = count($stimulusRating);
//  end
// $ratingCount = count($session->trials[0]->responses[0]->stimulus);
if($ratingCount > 1) {
